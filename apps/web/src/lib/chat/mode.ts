...
const EMOTIONAL_PATTERN =
  /好累|很累|真累|太累|有点累|有一?点累|有些累|有點累|有一點累|累了|累死|累瘫|累垮|疲惫|疲憊|难过|難過|难受|難受|心烦|好烦|烦死|烦躁|煩躁|焦虑|焦慮|崩溃|崩潰|失眠|睡不着|睡不好|压力大|压力好大|壓力大|委屈|想哭|emo|沮丧|沮喪|低落|郁闷|鬱悶|心慌|孤独|孤獨|撑不住|撐不住|心情不好|不开心|不開心|无力感|無力感|破防|内耗|內耗/;
...
export function modeInstruction(mode: ConversationMode): string {
  switch (mode) {
    ...
    case "casual":
    default:
      return `当前会话模式：日常闲聊。接住用户后，自己贡献一点有依据的观察、态度、联想或轻微玩笑，再自然留白；问句不是默认推进动作。不要总结、布置任务或把生活分享立刻变成建议。${MODE_FALLBACK}`;
  }
}
